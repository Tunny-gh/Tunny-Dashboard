...
- パレート前面の制約対応（現状: 目的空間の非劣解として計算し、制約違反は
  「（違反）」注記で透明化。feasible のみの前面計算オプションが望ましい）
- 前面内の同一目的値 trial の重複マーク（例: 同一パラメータの再サンプル）

### 3. UI 磨き込み

- ダークモード（現状ライトテーマのみ）
- UI 本体の日本語化（現状はヘルプ/理論ドキュメントのみバイリンガル）。
  国内 CAE ユーザー層に対して商用が弱い部分
- README の機能リスト更新（PROMETHEE / SOM / 応答曲面 / ロバスト性等が未記載）
