...
describe('StudyStore — 正常系', () => {
  beforeEach(() => {
    resetStore();
    vi.clearAllMocks();
    mockGetInstance.mockResolvedValue({ parseJournal: mockParseJournal, selectStudy: mockSelectStudy });
  });
  ...
  test('TC-302-09: loadJournal が parseJournal を呼び出し allStudies を更新する', async () => {
    ...
    const mockStudies = [
      { studyId: 1, name: 'study-1', directions: ['minimize'], completedTrials: 100, totalTrials: 100, paramNames: ['x'], objectiveNames: ['y'], userAttrNames: [], hasConstraints: false },
    ];
