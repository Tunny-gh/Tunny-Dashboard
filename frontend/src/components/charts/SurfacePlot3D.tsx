  ...
  if (result && result.grid1.length > 0 && result.grid2.length > 0) {
    ...
    for (let i = 0; i < result.grid1.length; i++) {
      for (let j = 0; j < result.grid2.length; j++) {
        const z = result.values[i][j]
        if (isFinite(z)) {
          surfaceData.push([result.grid1[i], result.grid2[j], z])
          if (z < minZ) minZ = z
          if (z > maxZ) maxZ = z
        }
      }
    ...
    chartOption = {
      ...
      visualMap: {
        ...
        formatter: (v: number) =>
          Math.abs(v) >= 1000 || Math.abs(v) < 0.01 ? v.toExponential(2) : v.toFixed(3),
      },
      grid3D: {
        boxWidth: 100,
        boxDepth: 100,
    ...
    >
      ...
      >
        ...
        {result && (
          <span
            style={{
              color: result.rSquared >= 0.5 ? '#27ae60' : '#e67e22',
              fontSize: '11px',
              alignSelf: 'center',
            }}
          >
