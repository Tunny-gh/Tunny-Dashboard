...
fn log10_grid_spacer(input: egui_plot::GridInput) -> Vec<egui_plot::GridMark> {
...
mod tests {
    ...
    fn format_log_tick_restores_original_scale() {
        // ln 変換した値を exp で戻して整形した結果が元の値に近い表記になる
    ...
    fn log10_grid_spacer_places_decades_as_majors() {
        // 1（10^0）〜1000（10^3）に相当する log10 範囲
        ...
        // 主目盛り（step_size == 1.0）は整数の log10 値（= 10 の累乗）に並ぶ
