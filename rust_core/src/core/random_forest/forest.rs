...
pub(crate) fn train_rf_on_columns(
    x_matrix: &[Vec<f64>],
    y: &[f64],
    column_indices: &[usize],
    n_trees: usize,
    ...
    let x_subset = extract_columns(x_matrix, column_indices)?;
    Some(RandomForest::train(
        &x_subset,
        y,
        n_trees,
        max_depth,
...
pub(crate) fn mse_on_dataset(rf: &RandomForest, x_matrix: &[Vec<f64>], y: &[f64]) -> Option<f64> {
