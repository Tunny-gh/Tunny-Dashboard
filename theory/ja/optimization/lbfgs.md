...
$$
\|\nabla \mathcal{L}\|_2 < \sqrt{\epsilon_{f64}} \approx 1.49 \times 10^{-8}
$$

これは argmin 0.11 の `tol_grad` のデフォルト値（コード側で `with_tolerance_grad` は呼び出していないため、argmin のデフォルトがそのまま適用される）。または最大イテレーション数に達した場合に停止。
