...
L-BFGS は**サロゲートオプティマイザ**段階で使用される。フィット済みの応答曲面（Gaussian Process・Sparse Gaussian Process・Ridge）上で目的を最小化または最大化するパラメータ値を探索する。数値勾配（中心差分）を使用するため、どのサロゲートモデルにも同じオプティマイザが適用できる。
