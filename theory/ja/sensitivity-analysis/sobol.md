...
> **有限サンプル推定の注意:** Saltelli 一次指数推定量と Jansen 全効果推定量は独立に計算されるため、有限サンプルでは $\hat{S}_i > \hat{ST}_i$ が生じることがある。実装では推定後に $\hat{ST}_i \leftarrow \max(\hat{ST}_i, \hat{S}_i)$ を適用してから $[0,1]$ にクランプし、理論的性質を出力値で保証している。
