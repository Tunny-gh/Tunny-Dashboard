...
GP-FITC または GP-VFE（ガウス過程）を使った 1D PDP では、**各グリッド点での予測分散**をトレーニングデータ全点で平均することが理論的に正確だが、計算コストが $O(G \times N \times N^2) = O(G \cdot N^3)$（$G$ = グリッド数、$N$ = 訓練点数）になる。
...
### コストを抑えているのは誘導点であり、訓練行のサブサンプリングではない

上記の平均計算（全 $N$ 訓練点にわたる完全 MC）は、全 $N \times N$ 訓練共分散に対して評価すると $O(G \cdot N \cdot N^2) = O(G N^3)$ になる。実際には GP-FITC / GP-VFE が $M \le 100$ 個の**誘導点（inducing points）**（[ガウス過程](../surrogate-models/gaussian-process.md) 参照）によってこのコストをあらかじめ抑えており、これは $N$ に依存しない FITC/VFE 近似そのものの性質である。**訓練行 $N$ のサブサンプリングではない**：平均計算は依然として全 $N$ 訓練点を周辺化する。

| 操作 | コスト（N=100, G=30） |
| --- | --- |
| GP 訓練（Cholesky、$M \le 100$ の誘導点で抑制） | $O(N^3) = 10^6$ 演算（1回） |
| 平均計算（全グリッド点・全 $N$ 訓練点） | $O(G \cdot N \cdot N^2) = 3 \times 10^7$（前進代入×3000回） |
| **分散計算（セントロイド近似）** | $O(G \cdot N^2) = 3 \times 10^5$（前進代入×30回） |

各グリッド点について全訓練点で分散を評価する場合（セントロイドで1点だけ評価する代わりに）と比べ、この例では**分散ステップだけでおよそ $N$ 倍高速化**される。
