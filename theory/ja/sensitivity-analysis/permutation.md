...
RF-ANOVA（同プロジェクトの既存実装、fANOVA）は葉ボックス上の分散分解であり、シャッフルを一切行わない。これに対し PFI は**シャッフルによる精度低下**そのものを重要度の根拠とする手法であり、**n_repeats=5 回の独立した並び替えを平均**することで重要度推定の統計的ばらつきを低減している。
...
両手法は同じホールドアウト分割シードを使うが、RF-ANOVA の重要度はシャッフルを介さず学習済みフォレストから直接算出される点が本質的に異なる。少数サンプルや分散の大きいデータでは Permutation の方が安定した結果が得られる。
