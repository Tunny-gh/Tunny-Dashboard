...
> **エッジケース:** フィルタリング後のサンプル数が 4 未満の場合、ホールドアウト分割は無効化され、評価は訓練データと同じデータで行われる。この状況では重要度スコアの信頼性が低下する。
...
Step 3: LightGBM RF 学習（1 回のみ）
  └── 訓練データで 100 本の回帰木を構築
      （学習したモデルは全特徴量・全 repeat の評価で共有する）
...
| 計算時間 | RF-ANOVA + 評価が約 5 倍（学習は 1 回で共有） | ベースライン |
...
計算コストは「RF 学習 1 回 ＋ P × n_repeats 回の評価（予測）」で構成される。
支配的な RF 学習は 1 回だけなので、全体の計算量は RF-ANOVA に追加のパーミュテーション評価
（約 n_repeats 倍）を加えた程度であり、特徴量ごとにモデルを再学習していた旧実装より大幅に高速。
評価は学習済み booster での予測のみで軽量。バックグラウンドスレッドで実行されるため UI はブロックされない。
