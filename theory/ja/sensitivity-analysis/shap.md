...
グローバル重要度は、訓練データ（80% スプリット）の各サンプルの $|\phi_j(x)|$ を平均し、合計が 1 になるよう正規化する。
...
| 測定対象       | 予測値への各パラメータの貢献量                   | **学習中**の分岐品質                       | **学習後**の葉ボックスに対する分散分解   |
...
訓練データ（80% スプリット、サイズ $N$）の各サンプル $x_i$ の局所 Shapley 値の絶対値を平均し、合計が 1 になるよう正規化する。ここで $\phi_j(x_i)$ は `predict_contrib` が返すアンサンブル全体（全木の合算値）としての寄与であり、木ごとに分解した値ではない：

$$
\tilde{\phi}_j = \frac{\displaystyle\frac{1}{N} \sum_{i=1}^{N} |\phi_j(x_i)|}{\displaystyle\sum_{j'} \frac{1}{N} \sum_{i=1}^{N} |\phi_{j'}(x_i)|}
$$
