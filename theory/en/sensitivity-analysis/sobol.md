...
```
ST_i = 1 − Var(E[Y | X_{~i}]) / Var(Y)
```

where X_{~i} means "all parameters except x_i."

ST_i ≥ S_i always holds. A large gap (ST_i − S_i) indicates strong interactions.
...
- S_i high, ST_i ≈ S_i → parameter has a large independent effect, few interactions.
