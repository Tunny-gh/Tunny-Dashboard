# Analysis Methods

## Optimization

- [L-BFGS (Limited-memory BFGS)](optimization/lbfgs.md)
- [**Acquisition Functions (EI / LCB)**](optimization/acquisition-functions.md)
- [**Expected Hypervolume Improvement (EHVI)**](optimization/ehvi.md)

## Surrogate Models

- [Overview](surrogate-models/overview.md)
- [Gaussian Process (GP-FITC / GP-VFE)](surrogate-models/gaussian-process.md)
- [Gaussian Process MoE (GP-MOE)](surrogate-models/gaussian-process-moe.md)
- [Ridge](surrogate-models/ridge.md)
- [Random Forest](surrogate-models/random-forest.md)

## Widgets

- [Surrogate Optimizer](widgets/surrogate-optimizer.md)
